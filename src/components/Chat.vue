    ...
    <template v-else>
      <div class="chat-top-bar dark_main_text dark_light_bg">
        <template v-if="!MultiOn">
          <div class="chatinfo">
            <span>{{ chatname }}</span>
            <span v-if="chatinfo.isFriend" class="info online">online</span>
            <span v-else class="info">{{ chatinfo.members.length }} members</span>
          </div>
        </template>
      ...
      <div class="chat-messages dark_deep_bg">
        ...
        <vueScroll ref="chatMessages" @handle-scroll="handleScroll">
          <div v-for="msg in msgList" :key="msg.id" :id="'msg' + msg.id" class="msg-wrapper">
            <!-- 多选框 -->
